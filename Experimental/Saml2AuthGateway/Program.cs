...
namespace Saml2AuthGateway
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Saml2IdentConfig? saml2 = new Saml2IdentConfig("Saml2");
            ...
            EnablerLog.LogStatic("Saml2AuthGateway", "program starting...");

            // Configure the HTTP request pipeline.
            if (app.Environment.IsDevelopment())
            {
                app.UseDeveloperExceptionPage();
                EnablerLog.LogStatic("Saml2AuthGateway", "app.UseDeveloperExceptionPage()");
            }
            else
            {
                ...
                app.UseHsts();
                EnablerLog.LogStatic("Saml2AuthGateway", "app.UseExceptionHandler(\"/Error\")");
            }
           
            app.UseStaticFiles();
            app.UseRouting();
            EnablerLog.LogStatic("Saml2AuthGateway", "app.UseRouting()");

            if (saml2_entityId.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
            {
                app.UseHttpsRedirection();
                EnablerLog.LogStatic("Saml2AuthGateway", "app.UseHttpsRedirection()");
            }
            app.UseAuthentication();
            app.UseAuthorization();
            app.UseCookiePolicy();
            EnablerLog.LogStatic("Saml2AuthGateway", "app.UseAuthorization().UseAuthorization()");

            app.MapBlazorHub();
            app.MapFallbackToPage("/_Host");
            EnablerLog.LogStatic("Saml2AuthGateway", "app.MapBlazorHub().MapFallbackToPage(\"/_Host\")");
