            ...
            get
            {
                lock (_lock)
                {
                    if (string.IsNullOrEmpty(_baseAppPath))
                    {
                        ...
                        {
                            if (File.Exists(Path.Combine(_baseAppPath, "appsettings.json")) &&
                                _baseAppPath.Contains(Path.DirectorySeparatorChar + "bin", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if (!string.IsNullOrEmpty(bpath) && Directory.Exists(bpath) && File.Exists(Path.Combine(bpath, "appsettings.json")))
                                    _baseAppPath = bpath;
                            }
                        }
                    }
